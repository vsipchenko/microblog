...
def profile(user_id):
    ...
    page = request.args.get('page', 1, type=int)
    posts = user.posts.order_by(Post.id.desc()).paginate(page, app.config['POSTS_PER_PAGE'], False)

    next_url = url_for('profile', user_id=user.id, page=posts.next_num) if posts.has_next else None
    prev_url = url_for('profile', user_id=user.id, page=posts.prev_num) if posts.has_prev else None
