...
def explore():
    page = request.args.get('page', 1, type=int)
    posts = Post.query.order_by(Post.id.desc()).paginate(page, current_app.config['POSTS_PER_PAGE'],
                                                         False)
...
def profile(user_id):
    ...
    page = request.args.get('page', 1, type=int)
    posts = user.posts.order_by(Post.id.desc()).paginate(page, current_app.config['POSTS_PER_PAGE'],
                                                         False)

    next_url = url_for('main.profile', user_id=user.id,
                       page=posts.next_num) if posts.has_next else None
    prev_url = url_for('main.profile', user_id=user.id,
...
def follow(user_id):
    ...
    db.session.commit()
    flash('You are following {}!'.format(user_id))
    return redirect(url_for('main.profile', user_id=user_id))
